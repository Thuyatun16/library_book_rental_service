...
describe('UserService', () => {
  ...
  describe('create', () => {
    it('should create a new user', async () => {
      ...
      const mockUser = { id: 'uuid-1', ...createUserDto, password: 'hashedPassword', createdAt: new Date(), updatedAt: new Date() };
  ...
  describe('findAll', () => {
    it('should return an array of users', async () => {
      const mockUsers = [
        ...
        { id: 'uuid-2', name: 'User 2', email: 'user2@example.com', role: Role.TEACHER, createdAt: new Date(), updatedAt: new Date() },
      ];
  ...
  describe('findOne', () => {
    it('should return a single user by ID', async () => {
      ...
      expect(prismaService.user.findUnique).toHaveBeenCalledWith({ where: { id: 'uuid-1' } });
      expect(result).toEqual(mockUser);
    });
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException if user not found for update', async () => {
      ...
      await expect(service.update('non-existent-uuid', {})).rejects.toThrow(NotFoundException);
    });
  });

  describe('remove', () => {
    it('should delete a user by ID', async () => {
      const mockDeletedUser = { id: 'uuid-1', name: 'User 1', email: 'user1@example.com', role: Role.STUDENT, createdAt: new Date(), updatedAt: new Date() };
      (prismaService.user.findUnique as jest.Mock).mockResolvedValue(mockDeletedUser); // User exists
      (prismaService.user.delete as jest.Mock).mockResolvedValue(mockDeletedUser);

      const result = await service.remove('uuid-1');
      expect(prismaService.user.findUnique).toHaveBeenCalledWith({ where: { id: 'uuid-1' } });
      expect(prismaService.user.delete).toHaveBeenCalledWith({ where: { id: 'uuid-1' } });
